...
void catmullrom2_tangent(float t, float* points, int npoints, float* x) {
  ...
  //  printf("catmull rom debug t,i,tp npoints: %f %d %f %d\n", t, i, tp, npoints);
    ...
    catmullrom2_segment_tangent(tp,p0,p1,p2,p3,alpha,x);
...
void catmullrom2_segment_tangent(float tp, float* p0, float* p1, float* p2, float* p3, float alpha, float* x) {
    ...
    weighted_sum2( &A1p[0], (-1.0/(t1-t0))  , (1.0/(t1-t0))   , p0, p1  );
    weighted_sum2( &A2p[0], (-1.0/(t2-t1))  , (1.0/(t2-t1))   , p1, p2  );
    weighted_sum2( &A3p[0], (-1.0/(t3-t2))  , (1.0/(t3-t2))   , p2, p3  );

    weighted_sum2( &B1[0], ((t2-t)/(t2-t0)), ((t-t0)/(t2-t0)), &A1[0], &A2[0]  );
    weighted_sum2( &B2[0], ((t3-t)/(t3-t1)), ((t-t1)/(t3-t1)), &A2[0], &A3[0]  );

//    weighted_sum2_4( &B1p[0], ((t2-t)/(t2-t0)), ((t-t0)/(t2-t0)), -1.0, 1.0, &A1p[0], &A2p[0], &A1[0], &A2[0]  );
//    weighted_sum2_4( &B2p[0], ((t3-t)/(t3-t1)), ((t-t1)/(t3-t1)), -1.0, 1.0, &A2p[0], &A3p[0], &A2[0], &A3[0]  );
    weighted_sum2( &B1p[0], ((t2-t)/(t2-t0)), ((t-t0)/(t2-t0)), &A1p[0], &A2p[0] );
    weighted_sum2( &B2p[0], ((t3-t)/(t3-t1)), ((t-t1)/(t3-t1)), &A2p[0], &A3p[0] );

//    weighted_sum2_4( x     , ((t2-t)/(t2-t1)), ((t-t1)/(t2-t1)), -1.0, 1.0, &B1p[0], &B2p[0], &B1[0], &B2[0]  );
    weighted_sum2( x     , ((t2-t)/(t2-t1)), ((t-t1)/(t2-t1)), &B1p[0], &B2p[0]  );
